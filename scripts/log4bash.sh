...
# declare -A LEVELS=( ["ERROR"]=10 ["WARNING"]=20 ["SUCCESS"]=30 ["INFO"]=40 ["DEBUG"]=50 )
ERROR_NUM=10
WARNING_NUM=20
SUCCESS_NUM=30
INFO_NUM=40
...
log() {
    ...
    log_level_var="${log_level}_NUM"
    log_level_num="${!log_level_var}"
    requested_level_var="${LOG_LEVEL}_NUM"
    requested_level_num="${!requested_level_var}"

    msg="${log_color}[$(date +"%Y-%m-%d %H:%M:%S %Z")] [${log_level}] ${log_text} ${LOG_DEFAULT_COLOR}";

    if [[ ${log_level_num} -le ${requested_level_num} ]]; then
        >&2 echo -e "$msg"
    fi
    return 0;
}
